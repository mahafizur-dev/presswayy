  ...
  try {
    ...
    if (!client_id || !plan) {
      return NextResponse.json(
        {
          success: false,
          error: "Missing required fields: client_id or plan",
        },
        { status: 400 },
      );
    }
    ...
    const tran_id = `PWY_SUB_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    ...
    const payload = {
      ...
      currency: "BDT",
      tran_id: tran_id,
      success_url: `${baseUrl}/api/subscription/success`,
      fail_url: `${baseUrl}/api/subscription/fail`,
      cancel_url: `${baseUrl}/api/subscription/cancel`,
      ipn_url: `${baseUrl}/api/subscription/ipn`,
      shipping_method: "No",
      product_name: "Presswayy Monthly Subscription",
